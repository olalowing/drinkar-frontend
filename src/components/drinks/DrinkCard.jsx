...
export default function DrinkCard({ drink }) {
  ...
  return (
    <Link to={`/drinks/${drink.id}`} className="group">
      <Card className="cursor-pointer h-full hover:scale-[1.02] hover:shadow-xl overflow-hidden">
        {/* Image */}
        {mainImage ? (
          <div className="relative h-48 -mt-6 -mx-6 mb-4 rounded-t-xl overflow-hidden">
            ...
            {drink.images.length > 1 && (
              <div className="absolute top-3 right-3 bg-black/70 backdrop-blur-sm text-white text-xs px-2.5 py-1.5 rounded-lg font-medium">
          ...
          <div className="relative h-48 -mt-6 -mx-6 mb-4 rounded-t-xl bg-gradient-to-br from-primary-100 via-primary-200 to-accent-100 flex items-center justify-center group-hover:from-primary-200 group-hover:via-primary-300 group-hover:to-accent-200 transition-all duration-300">
            <Wine className="w-16 h-16 text-primary-500 group-hover:scale-110 transition-transform duration-300" />
          </div>
        )}
        ...
        <div className="space-y-3">
          {/* Title */}
          <div>
            <h3 className="text-xl font-bold text-gray-900 line-clamp-1 group-hover:text-primary-700 transition-colors">
              {drink.name}
            </h3>
            {drink.spritbas && (
          ...
          {drink.tags && drink.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {drink.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="inline-block px-2.5 py-1 bg-gradient-to-r from-primary-100 to-primary-50 text-primary-800 text-xs font-semibold rounded-full border border-primary-200"
                >
                  {tag}
                </span>
              ))}
              {drink.tags.length > 3 && (
                <span className="inline-block px-2.5 py-1 bg-gray-100 text-gray-600 text-xs font-semibold rounded-full border border-gray-200">
                  +{drink.tags.length - 3}
                </span>
              )}
            </div>
          ...
          {/* Info */}
          <div className="flex items-center justify-between text-xs text-gray-600 pt-3 border-t border-gray-200/50">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-1.5" title="Glastyp">
                <GlassWater className="w-4 h-4 text-primary-500" />
